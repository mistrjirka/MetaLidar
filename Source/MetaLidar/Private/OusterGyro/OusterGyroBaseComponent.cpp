...
void UOusterGyroBaseComponent::calculateLinearFit(CircularBuffer<T, S> circBuffer, size_t size, FVector& vector_fit_a, FVector& vector_fit_b)
{
...
void UOusterGyroBaseComponent::TakeSnapshot(uint32 TimeStamp)
{
  ...
  this->calculateLinearFit(this->AccelerationBuffer, ACCELERATION_BUFFER_SIZE, this->linear_fit_a_vel, this->linear_fit_b_vel);
...
{
  return this->AccelerationBuffer.size() >= ACCELERATION_BUFFER_SIZE && this->RotationBuffer.size() >= ROTATION_BUFFER_SIZE;
}
